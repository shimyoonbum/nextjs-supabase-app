...
- [x] 권한에 따른 UI 조건부 렌더링 확인 (events/new 실제 인증 게이트로 교체)
- [x] 반응형 디자인 적용 완료
- [x] 다크 모드 지원 완료 (AdminSidebar 시맨틱 토큰 적용)
- [x] 사용자 플로우 네비게이션 동작 (중복 /join 라우트 제거, /invite로 통일)

#### Phase 3 (데이터베이스 설정 및 핵심 기능)
- [x] 데이터베이스 스키마 생성 및 RLS 설정 (보안·성능 권고 해소 마이그레이션 포함)
- [x] 모든 API 엔드포인트 구현 완료 (관리자 통계 실데이터 연동 포함)
- [x] 실시간 기능 정상 동작 (event_participants Realtime 구독 + 참여자 목록 실시간 갱신)
- [x] 인증 및 권한 시스템 동작 (Google OAuth, admin 미들웨어, verifyAdminAccess)
- [x] Playwright MCP 테스트 모두 통과 (미인증 플로우 8건 통과, 인증 플로우는 자격증명 설정 시 실행)
